...
intermediate_table <- as.data.frame(read_tsv("P_pyrhulla-SNPs_before_masking_fixed_hets.table", show_col_types=FALSE))
...
#for(primer in c(12)) {
            ...
            for(snp_row in possible_snp_rows) {
                ...
                if(length(longer_STR_allele_positions) == 2 && !any(is.na(longer_STR_allele_positions)) &&
                    ...
                    if(intermediate_table[snp_row, paste0(sample, "_STR_alleles_where_SNP_is_found")] == "0") {
                        ...
                        #special encoding for missing data
                        snp_position_column <- c("X", "X")
                    }
                    else {
                        snp_position_column <- c(alt_allele, alt_allele)
                    }
    ...
    write.table(overall_primer_output_data_frame, paste0("./finalExcelOutputs/Primer",primer,"_no_masking_SNPSTRs.tsv"),row.names=FALSE,
        quote=FALSE, sep="\t")
