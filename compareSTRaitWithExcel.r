...
for(threshold in c(0.15))
{
    ...
    strait_razor_genotypes <- as.data.frame(read_tsv(paste("strait_razor_genotypes_",
                                                    threshold,"_multiple_flanks.tsv",sep="")))
    ...
    print("Number of excel table entries for which STRait razor
                did not work")
    ...
    write.table(merged_genotype_df[which(no_equal_allele_vector),],
        paste("alleles/full_mismatch_STRait_razor_alleles_",threshold,"_multiple_flanks.tsv",sep=""),
        row.names=FALSE,quote=FALSE,sep="\t")

    write.table(merged_genotype_df[which(one_equal_allele_vector),],
        paste("alleles/single_mismatch_STRait_razor_alleles_",threshold,"_multiple_flanks.tsv",sep=""),
        row.names=FALSE,quote=FALSE,sep="\t")

    write.table(merged_genotype_df[c(which(no_equal_allele_vector),
                                    which(one_equal_allele_vector)),],
            paste("alleles/not_double_match_STRait_razor_alleles_",threshold,"_multiple_flanks.tsv",sep=""),
                row.names=FALSE,quote=FALSE,sep="\t")

     write.table(merged_genotype_df,
            paste("alleles/full_merged_STRait_excel_alleles_",threshold,"_multiple_flanks.tsv",sep=""),
                row.names=FALSE,quote=FALSE,sep="\t")
